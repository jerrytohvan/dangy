...
#DISPLAY: COUNTY, TOWNID, TOWNNAME, TOWNENG, total_cases, ....
selected_aggregated_temp <-pts.poly@data %>% dplyr::select(poly.ids, TOWNID,TOWNNAME, TOWNENG, Onset_day, Case_study_date, Notification_day) %>% 
  mutate(MONTH = months(as.Date(pts.poly@data$Onset_day))) %>%
  group_by(poly.ids,TOWNID,TOWNENG,MONTH) %>% summarise(total_cases = n())
selected_aggregated <- left_join(selected_aggregated_temp, taiwan_ts_map_sp@data, by=c("poly.ids"))
#https://shiny.rstudio.com/gallery/basic-datatable.html -> FILTERS

#info on the most post infected countries
