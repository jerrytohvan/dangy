...
#df_filtered <- df_dengue %>% filter(grepl(filter_year, Onset_day))
...
# Check for NA values for coordinates
 ...
 sf_dengue <- st_as_sf(df_dengue, 
                       coords = c("Minimum_statistical_area_center_point_X",
                        ...
                        crs = "+init=epsg:3826 +proj=tmerc +ellps=GRS80 +no_defs",na.fail=FALSE)
```

Map name dict to map
```{r}
#taiwan_ts_map_sf <-as(taiwan_ts_map_sf, 'Spatial')
proj4string(taiwan_ts_map_sp) <- CRS("+init=epsg:3826 +proj=tmerc +ellps=GRS80 +no_defs")
sf_dengue <- na.omit(sf_dengue)
transaction.boundary.poly <- point.in.poly(as(sf_dengue, 'Spatial'), taiwan_ts_map_sp)
